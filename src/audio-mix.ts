...
export async function mixAudio(
  videoAudio: AudioBuffer | null,
  bgmAudio: AudioBuffer | null,
): Promise<AudioBuffer | null> {
  const total = totalDuration();
  if (total <= 0) return null;
  if (!videoAudio && !bgmAudio) return null;
  ...
  if (videoAudio && state.videoVolume > 0) {
    let at = 0;
    for (const clip of state.clips) {
      const d = clipDuration(clip);
      if (d <= 0) continue;
      const src = ctx.createBufferSource();
      src.buffer = videoAudio;
      const gain = ctx.createGain();
      gain.gain.value = state.videoVolume;
      src.connect(gain).connect(ctx.destination);
      src.start(at, clip.start, Math.min(d, Math.max(0, videoAudio.duration - clip.start)));
      at += d;
    }
  }
  ...
  if (bgmAudio && bgm && bgm.volume > 0) {
    ...
    for (let t = step; t <= total; t += step) {
      gain.gain.linearRampToValueAtTime(
        bgmGainAt(t, total, bgm.volume, bgm.fadeIn, bgm.fadeOut),
        t,
      );
    }
    src.connect(gain).connect(ctx.destination);
